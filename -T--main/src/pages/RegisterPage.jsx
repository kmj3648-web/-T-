...
export default function RegisterPage() {
  ...
  const fetchStudents = async () => {
    ...
    if (error) {
      console.error(error);
      setErrorModal(parseError(error, '학생 명단 불러오기 실패'));
    } else {
      setStudents(data || []);
    }
    setLoading(false);
  ...
  const handleAddStudent = async (e) => {
    ...
    if (!student_name || !school || !grade || !birthdate) {
      alert('모든 필드를 입력해주세요.');
      return;
    }
    ...
    if (error) {
      console.error(error);
      setErrorModal(parseError(error, '개별 학생 등록 오류'));
    } else {
      alert('학생이 등록되었습니다.');
  ...
  const handleDelete = async (id) => {
    ...
    if (error) {
      console.error(error);
      setErrorModal(parseError(error, '학생 삭제 오류'));
    } else {
      fetchStudents();
    }
  };
  ...
  const handleBulkUpload = async () => {
    ...
    if (error) {
      console.error(error);
      setErrorModal(parseError(error, '학생 일괄 등록 오류'));
    } else {
