...
export default function AdminPage() {
  ...
  const fetchStudents = async () => {
    ...
    if (!error && data) {
      ...
      console.error(error);
      setErrorModal(parseError(error, '학생 명단 로딩 실패'));
    }
  };
  ...
  const handleChangePassword = async (e) => {
    ...
    if (error) {
      console.error(error);
      setErrorModal(parseError(error, '비밀번호 변경 실패'));
    } else {
  ...
  const fetchConfig = async () => {
    ...
    if (!error && data) {
      ...
      console.error(error);
      setErrorModal(parseError(error, '설정 로딩 실패'));
    }
  };
  ...
  const fetchBookings = async () => {
    ...
    if (!error && data) {
      ...
      console.error(error);
      setErrorModal(parseError(error, '스케줄 목록 불러오기 실패'));
    }
    setLoadings(false);
  };
  ...
  const onDrop = async (e, targetDate, targetTime, targetType) => {
    ...
    if (targetDate === 'TRASH') {
      if (confirm('선택하신 예약을 삭제/취소 하시겠습니까?')) {
        const { error } = await supabase.from('clinics').delete().eq('id', draggedId);
        if (error) {
          setErrorModal(parseError(error, '예약 삭제 실패'));
        } else {
          fetchBookings();
        }
      }
    ...
    const { count, error: capError } = await supabase.from('clinics').select('*', { count: 'exact', head: true })
      .eq('clinic_date', targetDate).eq('clinic_time', targetTime).eq('clinic_type', mode);
      
    if (capError) {
      setErrorModal(parseError(capError, '수용 인원 확인 실패'));
      return;
    }
    ...
    const { error: updateError } = await supabase.from('clinics').update({ clinic_date: targetDate, clinic_time: targetTime, clinic_type: mode }).eq('id', draggedId);
    if (updateError) {
      setErrorModal(parseError(updateError, '예약 이동 실패'));
    } else {
      fetchBookings();
    }
  };
  ...
  const handleAttendanceSync = async (e) => {
    ...
    try {
      ...
      bookings.forEach(b => {
        ...
        const timeName = getShortTime(b.clinic_time);
      ...
      console.error(err);
      setErrorModal(parseError(err, '엑셀 변환 중 오류 발생'));
    }
